...
def draw_ice():
    ...
    turtle.write("Zhuoqun Love Fan",font=('Ink Free', 16, 'normal'))
    turtle.goto(-40, -160)
    turtle.color('Black')
    turtle.write("Happy Valentine's Day", font=('Arial', 10, 'bold italic'))
    turtle.hideturtle()
    ...
    scale=0.2
    img_path = r'C:\Users\wufan\Pictures\2.png'
    ...
    turtle.goto(x_begin, y_begin)  # 移动到指定坐标的位置  -140，
    turtle.seth(0)
    turtle.pd()
    turtle.tracer(300)  # 加快作图速度， 0 or False 时图形一次性画好

    for k1, i in enumerate(img1):
        for j in i[::int(1/scale)]:
            turtle.pencolor((j[2], j[1], j[0]))  # 设置画笔颜色
            turtle.fd(1)  # 沿着海龟的前方向运行1
        turtle.pu()  # 画笔抬起，不留下痕迹
        turtle.goto(x_begin, y_begin-k1)  # 移动画笔到下一行最左侧位置
        turtle.pd()  # 画笔落下，留下痕迹
...
#     turtle.setup(width=width+100, height=height+100)  # 设置画布大小， 600， 370
#     turtle.pu()
#     turtle.goto(-width/2 + 10, height/2 - 10)  # 移动到指定坐标的位置  -140，
#     turtle.pd()
#     turtle.tracer(3000)  # 加快作图速度， 0 or False 时图形一次性画好
...
#             turtle.fd(1)  # 沿着海龟的前方向运行1
#         turtle.pu()  # 画笔抬起，不留下痕迹
#         turtle.goto(-width/2, height/2 -k1)  # 移动画笔到下一行最左侧位置
#         turtle.pd()  # 画笔落下，留下痕迹
#     turtle.done()

# path = r'C:\Users\wufan\Pictures\2.png'
# draw_img(path, 1)
